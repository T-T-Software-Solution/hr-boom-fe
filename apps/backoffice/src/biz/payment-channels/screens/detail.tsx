...
interface PaymentChannelDetailScreenProps {
  id: string;
  modalId?: string;
}

export const PaymentChannelDetailScreen: React.FC<
  PaymentChannelDetailScreenProps
> = ({ id, modalId }) => {
  ...
  const handleUpdate = () => {
    router.push(`/payment-channel/update/${id}`);
    if (modalId) {
    ...
    <Stack gap="xs">
      <Group gap="xs" align="center">
        <Title order={1} size="h3">
          รายละเอียดตั้งค่าช่องทางจ่ายเงินเดือน{' '}
        </Title>
